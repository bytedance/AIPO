...
if python3 -c "from transformers import AutoModelForCausalLM; AutoModelForCausalLM.from_pretrained('${LOG_ROOT}/${PROJECT_NAME}/${EXPERIMENT_NAME}/iter${ITERATION}/dpo')"; then
  ...
  alignment_run -m \
    ...
    ${SELF_INSTRUCT_STAGE_ADDITIONAL_ARGS} || exit 1

  alignment_run -m \
    +experiments=recipes/ultrafeedback-self_instruct-pair_rm/iter_0_stage_2_generate_responses \
    log.root="${LOG_ROOT}" log.project_name="${PROJECT_NAME}" log.experiment_name="${EXPERIMENT_NAME}" \
    ${GENERATE_RESPONSES_STAGE_ADDITIONAL_ARGS} || exit 1

  alignment_run -m \
    +experiments=recipes/ultrafeedback-self_instruct-pair_rm/iter_0_stage_3_pair_rm \
    log.root="${LOG_ROOT}" log.project_name="${PROJECT_NAME}" log.experiment_name="${EXPERIMENT_NAME}" \
...
for ITER in $(seq 1 "$MAX_ITER"); do
  ...
  alignment_run -m \
    +experiments=recipes/ultrafeedback-self_instruct-pair_rm/iter_n_stage_2_generate_responses \
    log.root="${LOG_ROOT}" log.project_name="${PROJECT_NAME}" log.experiment_name="${EXPERIMENT_NAME}" \
    ${GENERATE_RESPONSES_STAGE_ADDITIONAL_ARGS} || exit 1

  alignment_run -m \
    +experiments=recipes/ultrafeedback-self_instruct-pair_rm/iter_n_stage_3_pair_rm \
    log.root="${LOG_ROOT}" log.project_name="${PROJECT_NAME}" log.experiment_name="${EXPERIMENT_NAME}" \
    ${PAIR_RM_STAGE_ADDITIONAL_ARGS} || exit 1
